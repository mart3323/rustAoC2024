...
const DAY: &str = "day14 - Restroom Redoubt";
    ...
    match robots {
        ...
        Ok(mut robots) => {
            for i in 1..10000 {
                ...
                let dir = Path::new("src").join(DAY).join("images");
                fs::create_dir(&dir); // if it fails, assumje dir already exists
                image.save(&dir.join(format!("{}.png", i))).unwrap();
