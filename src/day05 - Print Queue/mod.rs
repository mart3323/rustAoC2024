...
const DAY: &'static str = "day05 - Print Queue";
...
fn test_solve_simple() {
    let (rules, updates) = parse_file(&read_input_file(DAY, "demo.txt")).expect("Demo file should parse").1;
...
pub fn part1() -> usize {
    let (rules, updates) = parse_file(&read_input_file(DAY, "full.txt")).expect("Full file should parse").1;
